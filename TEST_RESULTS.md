...
**Run at:** 2026-05-10T14:50:24.369478Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 193ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 147ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "2FcHZ6CJTTqUd |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 254ms | 0 services |
| ✗ GET /api/brand | 404 | 149ms | phone: None |
| ✗ GET /api/i18n | 404 | 149ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 148ms | booking_id: None |

## ❌ #6: Anonymous: book AC cleaning 4 units
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 127ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 128ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 227ms | vendor_id=None, token len=0 |
