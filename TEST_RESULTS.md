...
**Run at:** 2026-05-16T19:23:04.019772Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 81ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 31ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "j0CtB8Y6QzStP |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 45ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 42ms | 0 services |
| ✗ GET /api/brand | 404 | 34ms | phone: None |
| ✗ GET /api/i18n | 404 | 76ms | 4 languages |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 31ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| ✗ GET /admin/vendors | 404 | 55ms | 0 vendors |
| ✗ GET /admin/services-summary | 404 | 35ms | 0 services |

## ❌ #18: Admin: read brand, no-op patch (preserve existing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 28ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 40ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 39ms | 0 vendors offering |
