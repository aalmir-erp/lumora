...
**Run at:** 2026-05-07T08:48:09.449377Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 133ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 98ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "fpjx9w5bRM-Ln |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 91ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 82ms | 0 services |
| ✗ GET /api/brand | 404 | 83ms | phone: None |
| ✗ GET /api/i18n | 404 | 97ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 87ms | booking_id: None |

## ❌ #6: Anonymous: book AC cleaning 4 units
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 90ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 88ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 111ms | vendor_id=None, token len=0 |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 229ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 96ms | 0 vendors |
| ✗ GET /admin/services-summary | 404 | 91ms | 0 services |

## ❌ #18: Admin: read brand, no-op patch (preserve existing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 100ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 88ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 86ms | 0 vendors offering |

## ❌ #20: Public: GET /api/reviews/deep_cleaning
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/reviews/deep_cleaning | 404 | 81ms | count=None, avg=None |
