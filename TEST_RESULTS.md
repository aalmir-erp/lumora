...
**Run at:** 2026-05-06T06:55:28.382175Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 62ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 18ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "NmDoy3rXSe6XC |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 21ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 18ms | 0 services |
| ✗ GET /api/brand | 404 | 18ms | phone: None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 30ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 17ms | vendor_id=None, token len=0 |
...
| ✗ GET /admin/services-summary | 404 | 21ms | 0 services |
