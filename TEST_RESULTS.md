...
**Run at:** 2026-05-05T20:52:02.152238Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 132ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 100ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "psa97floRJ2iN |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 137ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 75ms | 0 services |
| ✗ GET /api/brand | 404 | 109ms | phone: None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 105ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 95ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 119ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 103ms | 0 vendors |
...
| ✗ POST /admin/brand (no-op) | 404 | 92ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 108ms | 0 vendors offering |
