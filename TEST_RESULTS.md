...
**Run at:** 2026-05-05T20:59:19.315326Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 111ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 83ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "UbnaelXoRvCCZ |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 88ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 81ms | 0 services |
| ✗ GET /api/brand | 404 | 87ms | phone: None |
| ✗ GET /api/i18n | 404 | 78ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 92ms | booking_id: None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 80ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 87ms | vendor_id=None, token len=0 |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 83ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 82ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 82ms | 0 vendors offering |

## ❌ #20: Public: GET /api/reviews/deep_cleaning
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/reviews/deep_cleaning | 404 | 83ms | count=None, avg=None |
