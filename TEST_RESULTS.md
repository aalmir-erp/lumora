...
**Run at:** 2026-05-06T18:57:57.055186Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 44ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 17ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "Tok3cnaIR1aLg |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 39ms | 0 services |
| ✗ GET /api/brand | 404 | 21ms | phone: None |
| ✗ GET /api/i18n | 404 | 168ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 18ms | booking_id: None |

## ❌ #6: Anonymous: book AC cleaning 4 units
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 17ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 45ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 22ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 17ms | 0 vendors |
| ✗ GET /admin/services-summary | 404 | 42ms | 0 services |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 18ms | 0 vendors offering |
