...
**Run at:** 2026-05-06T10:30:03.549454Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 98ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 38ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "-0ple1s2SmayZ |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 95ms | Sharjah covered: None |
...
| ✗ GET /api/i18n | 404 | 128ms | 4 languages |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 12ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 16ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 99ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 16ms | 0 vendors |
...
| ✗ POST /admin/brand (no-op) | 404 | 37ms | null |
