...
**Run at:** 2026-05-10T20:03:29.321186Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 65ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 18ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "kFL-WKULSkuzG |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 19ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 19ms | 0 services |
| ✗ GET /api/brand | 404 | 17ms | phone: None |
| ✗ GET /api/i18n | 404 | 21ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 14ms | booking_id: None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 24ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 19ms | vendor_id=None, token len=0 |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 15ms | bookings_today=None, total=None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 17ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 20ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 162ms | 0 vendors offering |

## ❌ #20: Public: GET /api/reviews/deep_cleaning
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/reviews/deep_cleaning | 404 | 29ms | count=None, avg=None |
