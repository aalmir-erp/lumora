...
**Run at:** 2026-05-06T06:08:20.396230Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 117ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 35ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "H_cGZLhkRnetu |

## ❌ #3: Anonymous: coverage check Sharjah
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (coverage) | 404 | 14ms | Sharjah covered: None |

## ❌ #4: Public: GET /api/services + /api/brand + /api/i18n
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 34ms | 0 services |
| ✗ GET /api/brand | 404 | 83ms | phone: None |
| ✗ GET /api/i18n | 404 | 15ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 16ms | booking_id: None |

## ❌ #6: Anonymous: book AC cleaning 4 units
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 14ms | booking_id: None |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 35ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 36ms | 0 vendors |
| ✗ GET /admin/services-summary | 404 | 13ms | 0 services |

## ❌ #18: Admin: read brand, no-op patch (preserve existing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 19ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 11ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 43ms | 0 vendors offering |
