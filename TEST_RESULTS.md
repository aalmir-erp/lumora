...
**Run at:** 2026-05-05T22:57:38.144923Z
...
|---|---|---|---|
| ✗ POST /api/chat | 404 | 130ms | text len=0, has AED: False, has tool_call: False |

## ❌ #2: Anonymous: deep clean 3BR with addons (oven, fridge)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (with addons) | 404 | 45ms | total: None AED — err: {"status": "error", "code": 404, "message": "Application not found", "request_id": "e0_N5_LZSSOty |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/services | 404 | 35ms | 0 services |
| ✗ GET /api/brand | 404 | 44ms | phone: None |
| ✗ GET /api/i18n | 404 | 33ms | 4 languages |

## ❌ #5: Anonymous: book general cleaning 2BR via chat
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (book) | 404 | 24ms | booking_id: None |

## ❌ #6: Anonymous: book AC cleaning 4 units
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /api/chat (AC book) | 404 | 13ms | booking_id: None |

## ❌ #7: Customer: OTP request + verify (login flow)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ OTP issue | 404 | 19ms | no dev_otp returned (WhatsApp bridge active or production mode) |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ POST /auth/vendor/login | 404 | 13ms | vendor_id=None, token len=0 |
...
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/stats | 404 | 14ms | bookings_today=None, total=None |
| ✗ GET /admin/vendors | 404 | 11ms | 0 vendors |
| ✗ GET /admin/services-summary | 404 | 15ms | 0 services |

## ❌ #18: Admin: read brand, no-op patch (preserve existing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/brand | 404 | 35ms | phone=None |
| ✗ POST /admin/brand (no-op) | 404 | 22ms | null |

## ❌ #19: Admin: service detail (vendors + pricing)
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /admin/service/deep_cleaning | 404 | 47ms | 0 vendors offering |

## ❌ #20: Public: GET /api/reviews/deep_cleaning
| Step | Code | Time | Result |
|---|---|---|---|
| ✗ GET /api/reviews/deep_cleaning | 404 | 11ms | count=None, avg=None |
