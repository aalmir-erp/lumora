...
### W0. NEVER USE RAILWAY ENV VARS FOR ANY CONFIGURATION ⛔ (added v1.24.230)

**ABSOLUTE BAN.** Founder mandate, verbatim: *"never ever use railway
for any any variable update this in md file also."*

Every single configurable value MUST be settable from the **admin UI**
inside Servia itself. Period. No exceptions. No "set this env var in
Railway then redeploy" instructions, ever.

If a setting needs to be configurable:
1. Add a row to `db.cfg` (the `config` table — already exists, persists
   across restarts, runtime editable)
2. Add an admin endpoint at `/api/admin/<feature>/<setting>` (GET to
   read, POST to write — both guarded by `require_admin`)
3. Add a tile / row in admin.html so the founder can see + change it
   without leaving the admin panel
4. Default to a sensible value in code (e.g. `db.cfg_get("key", "default")`)

Existing env-var lookups that should be migrated when touched:
- `ADMIN_WA_NUMBER` → done (v1.24.230 — DB takes precedence)
- `ADMIN_TOKEN` → bootstrap-only, never asked of the founder
- `WA_BRIDGE_TOKEN` → auto-generated by start.sh, founder never sees it
- `ANTHROPIC_API_KEY` → already migrated via AI Router admin UI
- `STRIPE_SECRET_KEY` / `ZIINA_API_KEY` → admin Payments tab already saves these
- `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON` → must be admin-UI input (in progress)

**Process for every new config value going forward:**
- ❌ NEVER tell the founder "set this env var in Railway and redeploy"
- ✅ ALWAYS build a 2-line admin UI for it (input + Save button), then
  reference db.cfg in the consumer code

If a setting MUST be at boot time (truly impossible to fetch from DB
after startup — extremely rare, e.g. DATABASE_URL itself), document
WHY in code with a comment explaining the boot-order constraint.
Otherwise: db.cfg always.

This rule supersedes any pattern in older code. Every time you touch
old code that reads from env, migrate the lookup to db.cfg-first.
