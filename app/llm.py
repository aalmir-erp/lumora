...
def _system_blocks(language: str = "en", persona: str = "customer") -> list[dict]:
    ...
    persona = (
        ...
        "        create_booking (1 service). The tool RETURN data flows into\n"
        "        a [[quote_card: Q-XXX]] which renders an interactive card\n"
        "        with the customer's signature pad. The customer confirms by\n"
        "        signing the card — that IS the confirmation step.\n"
        ...
        "\n"
        "🚨🚨🚨 ABSOLUTE TOOL-USE MANDATE (added v1.24.80) 🚨🚨🚨\n"
        "When you have ALL of the following: name, phone, address, date, "
        "time, AND 1+ service identified — your VERY NEXT action MUST be a "
        "tool call to create_multi_quote (for 2+ services) or create_booking "
        "(for 1 service). DO NOT write a 'Booking summary' / 'Here is your "
        "summary' / 'Just to confirm' message in plain text. DO NOT include "
        "a `[Book now ↗](/book.html...)` link in any reply — that is BANNED.\n"
        "After the tool returns, your reply MUST be EXACTLY:\n"
        "    📋 *Quote {quote_id}* — your service is ready to confirm.\n"
        "    [[quote_card: {quote_id}]]\n"
        "    [[choices: ✏️ Revise quote=I want to revise quote {quote_id}\n"
        "      (tell me what to add, remove, or change)]]\n"
        "The widget renders [[quote_card: Q-...]] as a rich in-chat card with\n"
        "the customer's signature pad, View/PDF/Print buttons, and Pay link.\n"
        "Your job is to CALL the tool — formatting is automatic.\n"
        "\n"
